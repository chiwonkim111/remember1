...
export default function AiSyncScreen({ navigate, onSyncContent }: Props) {
    ...
    <div className="flex-1 h-full overflow-y-auto no-scrollbar bg-[#F5F5F5]">
      ...
      <div className="pb-6 pt-3">
            ...
            {TOOLS.map(tool => (
              <div
                ...
                className={`flex items-center gap-3 p-3.5 rounded border cursor-pointer active:opacity-80 transition-opacity ${
                  tool.connected ? 'bg-white border-black' : 'bg-white border-[#E0E0E0]'
                }`}
              >
                {/* 브랜드 컬러 아이콘 */}
                <div
                  className="w-10 h-10 rounded-lg flex items-center justify-center text-white text-[14px] font-black shrink-0"
                ...
                <div className={`text-[11px] font-black px-2.5 py-1 rounded shrink-0 ${
                  tool.connected ? 'bg-black text-white' : 'border border-[#E0E0E0] text-[#AAAAAA]'
                }`}>
        ...
        <div className="mx-4 mb-4 bg-white border border-[#E0E0E0] rounded p-4">
          ...
          />
          <button
            className={`w-full mt-2.5 text-white text-[14px] font-black py-3 rounded cursor-pointer active:opacity-80 transition-opacity flex items-center justify-center gap-2 ${
              loading ? 'bg-[#AAAAAA]' : 'bg-[#FE5314]'
            }`}
        ...
        <ResultCard
          ...
          content="VINAIDA 앱의 AI 상담 시스템 고도화 완료. GPT-4 프롬프트 최적화로 API 비용 30% 절감 + 사용자 응답 만족도 4.2 → 4.7점 향상 달성."
          tags={["GPT-4", "프롬프트 엔지니어링", "MAKE", "Notion API"]}
          added={added.has("vinaida-ai")}
          onAdd={(content) => handleAdd("vinaida-ai", content)}
          onNavigateProfile={() => navigate?.(0)}
        />
        ...
        {synced && (
          <ResultCard
            ...
            content="VINAIDA 2호점 확장 기획 완료. 신규 입지 선정 및 운영 전략 수립, 예상 월매출 300만원 목표 설정."
            tags={["시장 분석", "사업 기획", "운영 전략"]}
            added={added.has("vinaida-2")}
            onAdd={(content) => handleAdd("vinaida-2", content)}
            onNavigateProfile={() => navigate?.(0)}
          />
        )}
...
function ResultCard({
  cardKey, title, period, role, result, content, tags, added, onAdd, onNavigateProfile,
}: {
  cardKey: string; title: string; period: string; role: string;
  result: string; content: string; tags: string[];
  added: boolean; onAdd: (content: string) => void; onNavigateProfile: () => void;
}) {
  return (
    <div className="mx-4 mb-3 bg-white rounded-lg border border-[#E0E0E0] overflow-hidden">
      <div className="px-4 py-3 border-b border-[#EBEBEB] flex items-center gap-2">
        <div className="w-[3px] h-5 rounded-full bg-[#FE5314] shrink-0" />
      ...
      <div className="px-4 py-3 flex flex-col gap-2">
        ...
        <div className="flex gap-3 items-start">
          ...
          <div className="flex flex-wrap gap-1.5 flex-1">
            {tags.map(tag => (
              <span key={tag} className="text-[11px] font-bold border border-[#E0E0E0] text-black px-2 py-0.5 rounded">{tag}</span>
            ))}
          </div>
        </div>
      </div>
      <div className="px-4 pb-4 flex flex-col gap-2">
        ...
        {added && (
          <button
            className="w-full text-[12px] font-black text-[#FE5314] py-1 cursor-pointer"
