...
export default function MarketScreen({ navigate }: { navigate?: (id: number) => void }) {
  return (
    <div className="flex-1 h-full overflow-y-auto no-scrollbar bg-[#F5F5F5]">

      {/* Header */}
      <div className="bg-black px-5 pt-6 pb-10">
        <div className="text-[10px] font-black text-[#FE5314] uppercase tracking-[0.2em] mb-3">시장가치 · 2026 Q1</div>
        <div className="text-[44px] font-black text-white leading-none tracking-tighter">
          8,500<span className="text-[24px] font-bold text-white/60">만원</span>
        </div>
        <div className="text-[13px] text-white/40 mt-2 font-medium">예상 범위 7,200 — 9,500만원</div>
      ...
      <div className="pb-6">
        ...
        <div className="-mt-5 mx-4 bg-white rounded-lg border border-[#E0E0E0] py-5 px-4 relative z-10 flex flex-col items-center">
          ...
          <div className="flex justify-between w-full mt-3">
            {[
              { val: '7,200만', sub: '최솟값', accent: false },
              { val: '8,500만', sub: '현재 추정', accent: true },
              { val: '9,500만', sub: '최댓값', accent: false },
            ].map(({ val, sub, accent }) => (
        ...
        <div className="px-4 mt-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              { val: '8,500만', sub: '나 (상위 32%)', highlight: true },
              ...
              >
                <div className={`text-[16px] font-black ${highlight ? 'text-[#FE5314]' : 'text-black'}`}>{val}</div>
        ...
        <div className="px-4 mt-4">
          ...
          <div className="bg-white rounded-lg border border-[#E0E0E0] p-4">
            {[
              ...
              <div key={name} className={`flex items-center gap-3 py-2.5 ${i < arr.length - 1 ? 'border-b border-[#EBEBEB]' : ''}`}>
                ...
                <div className="flex-1 h-[3px] bg-[#EBEBEB] rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${accent ? 'bg-[#FE5314]' : 'bg-black'}`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
                <div className={`text-[12px] font-black w-9 text-right shrink-0 ${accent ? 'text-[#FE5314]' : 'text-black'}`}>
                  {pct}%
                </div>
              </div>
            ))}
            <div className="mt-3 pt-3 border-t border-[#EBEBEB] text-[12px] text-[#666666] leading-snug">
              ...
              <span className="font-black text-[#FE5314]">글로벌 서비스 경험</span> 추가 시<br />
              예상 연봉 <span className="font-black text-black">+12%</span> 상승 가능
            </div>
          </div>
        </div>
